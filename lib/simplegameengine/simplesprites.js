...
function getRectSpriteBoundingRect(sprite) {
  ...
  if (sprite.img && sprite.img.ready) {
    ...
    let left = sprite.location.x - sprite.width / 2
    let top = sprite.location.y - height / 2
    let right = left + sprite.width
    let bottom = top + height
    let centerX = left + sprite.width / 2
    let centerY = top + height / 2
    rect = {
      width: sprite.width,
      height,
    ...
    rect = {
      ...
      right: sprite.location.x + sprite.width,
      bottom: sprite.location.y + sprite.height,
      centerX: sprite.location.x + sprite.width / 2,
      centerY: sprite.location.y + sprite.height / 2
    }
  }
  return rect
}
