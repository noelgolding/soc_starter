...
class Sprite {
    ...
    if (this.shape && this.shape == CIRCLE) {
      if (!this.cfg.getRawValue('radius')) {
        if (!this.cfg.getRawValue('width')) {
          ...
          this.cfg.set('radius', () => this.cfg.get('width') / 2)
        }
      }
  ...
  respawn() {
    ...
    this.angle = 0
    this.location = this.cfg.get('location').clone()
    if (this.shape && this.shape == RECT) {
      let loc = this.cfg.get('location')
      if ([TOP_CENTER, BOTTOM_CENTER].includes(loc)) {
        this.location.x = this.location.x - this.width / 2
        this.location.y = (TOP_CENTER == loc) ? 0 : this.location.y - this.height
    ...
    this.previousDirection = this.direction.clone()
  ...
  draw() {
    ...
    if (this.img && this.img.ready) {
      ...
      if (this.flipY && this.direction.y !== 0) {
        let yscale = (this.direction.y < 0) ? 1 : -1
        ctx.scale(1, yscale)
      }
...
class PowerUpSprite extends Sprite {
  constructor(config) {
    ...
    if (isNaN(this.cfg.get('value'))) {
      this.cfg.set('value', () => Math.floor(this.cfg.get('width') / 4))
    }
  }
}
...
class EnemySprite extends Sprite {
  constructor(config) {
    ...
    super(new SimpleGameConfig(default_cfg, config))
    if (isNaN(this.cfg.get('value'))) {
      this.cfg.set('value', () => this.cfg.get('width'))
    }
    this.readyToShoot = false
...
class SpriteGrid extends Sprite {
  constructor(config) {
    ...
    this.location.y += this.margin_top
    this.location.x += this.margin_left
  ...
  add(...sprites) {
    for (let sprite of sprites) {
      ...
      sprite.location = this.location.clone().add(new Vector(x_offset, y_offset));
      // console.log(`row idx: ${row_index}, col idx: ${col_index}, x_offset: ${x_offset}, y_offset: ${y_offset}, location: ${sprite.location}`)
...
function drawCircleSprite(sprite) {
  ctx.save()
  ctx.translate(sprite.location.x, sprite.location.y)
...
function getCircleSpriteBoundingRect(sprite) {
  ...
  if (sprite.img && sprite.img.ready) {
    ...
    let left = sprite.location.x - width / 2
    let top = sprite.location.y - height / 2
    ...
    rect = {
      width: sprite.r * 2,
      height: sprite.r * 2,
      left: sprite.location.x - sprite.r,
      top: sprite.location.y - sprite.r,
      right: sprite.location.x + sprite.r,
      bottom: sprite.location.y + sprite.r,
      centerX: sprite.location.x,
      centerY: sprite.location.y
    }
  }
  return rect
}
...
function getRectSpriteBoundingRect(sprite) {
  ...
  // let left = sprite.location.x - width / 2
  // let top = sprite.location.y - height / 2
  let left = sprite.location.x
  let top = sprite.location.y
  let right = left + width
  let bottom = top + height
  let centerX = left + width / 2
  let centerY = top + height / 2
