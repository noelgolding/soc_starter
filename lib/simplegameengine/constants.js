...
/**
 * @namespace constants.locs
 */
/**
 * @memberof constants.locs
 * @instance
 * @type {Vector}
 */
...
const BOTH_ID = 'can collide with both the player or the enemy'
 ...
 * @property {Vector | function} loc A Vector or function that returns a Vector. The (x,y) of the center of the sprite. Defaults to the center of the canvas. see {@link constants.locs}
 ...
 * @property {Vector | function} [loc=MIDDLE_CENTER] A Vector or function that returns a Vector. The (x,y) of the center of the sprite. Defaults to the center of the parent sprite. see {@link constants.locs}
 ...
 * @property {string} image An image that will be displayed for this sprite.
 * @property {Vector | function} [loc=MIDDLE_CENTER] A Vector or function that returns a Vector. The (x,y) of the center of the sprite. Defaults to the center of the canvas. see {@link constants.locs}
 * @property {number} [width=image width] The width of the sprite.
 * @property {number} [height=image height] The height of the sprite.
 ...
 * @extends simplegameengine.config.SimpleSpriteConfig
 * @property {collections.List | string} image An image or a List of images that will be displayed for this sprite. If a list is provided, one image will be randomly chosen.
 * @property {Vector | function} [loc=createRandomCanvasVector] A Vector or function that returns a Vector. The (x,y) of the center of the sprite. Defaults to the center of the canvas. see {@link constants.locs}
 * @property {number} [width=image width] The width of the sprite.
 * @property {number} [height=image height] The height of the sprite.
 * @property {boolean} [display=true] Whether or not to display the sprite.
 * @property {Vector | function} [direction=RIGHT] A Vector or function that returns a Vector. The direction the sprite is facing. see {@link constants.directions }
...
function wrapEdges(sprite) {
    ...
    let left_adjust = (sprite.shape == CIRCLE) ? rect.width / 2 : 0
    let right_adjust = (sprite.shape == CIRCLE) ? rect.width / 2 : rect.width
    let top_adjust = (sprite.shape == CIRCLE) ? rect.height / 2 : 0
    let bottom_adjust = (sprite.shape == CIRCLE) ? rect.height / 2 : rect.height
        // check top and bottom
    if (rect.bottom <= 0) {
        sprite.loc.y = canvas.height + top_adjust
    } else if (rect.top >= canvas.height) {
        sprite.loc.y = 0 - bottom_adjust
    }
    // check right and left
    if (rect.right <= 0) {
        sprite.loc.x = canvas.width + left_adjust
    } else if (rect.left >= canvas.width) {
        sprite.loc.x = 0 - right_adjust
    }
}
...
function stopAtEdges(sprite) {
    ...
    let bottom_adjust = (sprite.shape == CIRCLE) ? rect.height / 2 : rect.height
        // check top and bottom
    if (rect.top <= 0) {
        sprite.loc.y = 0 + top_adjust
        sprite.stop()
    } else if (rect.bottom >= canvas.height) {
        sprite.loc.y = canvas.height - bottom_adjust
        sprite.stop()
    }
    // check left and right
    if (rect.left <= 0) {
        sprite.loc.x = 0 + left_adjust
        sprite.stop()
    } else if (rect.right >= canvas.width) {
        sprite.loc.x = canvas.width - right_adjust
        sprite.stop()
    }
}
