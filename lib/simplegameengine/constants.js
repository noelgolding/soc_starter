...
function wrapEdges(sprite) {
  ...
  if (rect.bottom <= 0) {
    sprite.location.y = canvas.height + top_adjust
  } else if (rect.top >= canvas.height) {
    sprite.location.y = 0 - bottom_adjust
  }
  // check right and left
  if (rect.right <= 0) {
    sprite.location.x = canvas.width + left_adjust
  } else if (rect.left >= canvas.width) {
    sprite.location.x = 0 - right_adjust
  }
}
...
function stopAtEdges(sprite) {
  ...
  // check top and bottom
  if (rect.top <= 0) {
    sprite.location.y = 0 + top_adjust
    sprite.stop()
  } else if (rect.bottom >= canvas.height) {
    sprite.location.y = canvas.height - bottom_adjust
    sprite.stop()
  }
  // check left and right
  if (rect.left <= 0) {
    sprite.location.x = 0 + left_adjust
    sprite.stop()
  } else if (rect.right >= canvas.width) {
    sprite.location.x = canvas.width - right_adjust
    sprite.stop()
  }
}
