...
class Game {
  constructor(config) {
    let default_cfg = {
      ...
      fps: 60,
      keyPressedHandler: (key_code) => {},
      keyReleasedHandler: (key_code) => {},
      update: (self, delta) => {},
      paused: false,
      width: 640,
      height: 480,
    ...
    for (let item of this.gameObjects[BOTH_ID]) {
      for (let player of this.gameObjects[PLAYER_ID]) {
        if (item.intersects(player)) {
          player.collisionHandler(player, item)
          item.collisionHandler(item, player)
        }
      }
      for (let enemy of this.gameObjects[ENEMY_ID]) {
        if (item.intersects(enemy)) {
          enemy.collisionHandler(enemy, item)
          item.collisionHandler(item, enemy)
        }
      }
    }
  }
