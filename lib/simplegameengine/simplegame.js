class Game{
    ...
    for (let sprite of this.gameObjects[POWER_UP_ID]) {
      for (let player of this.gameObjects[PLAYER_ID]) {
        if (player.collidesWith(sprite)){
          sprite.collisionHandler({src: sprite, target: player})
          player.collisionHandler({src: player, target: sprite})
        }
      }
    }
    ...
    for (let bullet of this.gameObjects[BULLET_ID]) {
      for (let enemy of this.gameObjects[ENEMY_ID]) {
        if (bullet.collidesWith(enemy)){
          enemy.collisionHandler({src: enemy, target: bullet})
          bullet.collisionHandler({src: bullet, target: enemy})
        }
      }
    }
  ...
  start() {
    console.log('TODO: call loading, preload, setup')
    this.timestart = Date.now()
    this._gameloop()
  }
}
